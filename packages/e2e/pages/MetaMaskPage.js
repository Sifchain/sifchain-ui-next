...
export class MetaMaskPage {
  ...
  async importAccount() {
    ...
    await this.page.type(".first-time-flow__seedphrase input", this.config.options.mnemonic);
  ...
  async addNetwork() {
    await this.page.goto(`chrome-extension://${this.config.id}/home.html#settings/networks`);
    ...
    await this.page.type("#rpc-url", `http://localhost:${this.config.network.port}`);
  ...
  async reset() {
    ...
    await this.page.goto(`chrome-extension://${this.config.id}/home.html#settings/advanced`, {
      waitUntil: "domcontentloaded",
    });
    await this.page.waitForTimeout(1000);
    await this.page.click('[data-testid="advanced-setting-reset-account"] button');
    await this.page.waitForTimeout(1000);
