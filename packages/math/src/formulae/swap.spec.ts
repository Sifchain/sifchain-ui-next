...
describe("swap", () => {
  test.each(swapFixture.map((x) => [x.x, x.X, x.Y, x.expected]))(
    ...
    (fromAmount, fromCoinPoolAmount, toCoinPoolAmount, expected) => {
      const output = calculateSwapResult(fromAmount, fromCoinPoolAmount, toCoinPoolAmount);
      expect(output.toPrecision(18)).toEqual(new BigNumber(expected).toPrecision(18));
    },
  );
});
...
describe("swap with PMTP", () => {
  test.each(pmtpSwapFixture.map((x) => [x.x, x.X, x.Y, x.A, x.expected]))(
    ...
    (fromAmount, fromCoinPoolAmount, toCoinPoolAmount, adjustment, expected) => {
      const output = calculateSwapResult(fromAmount, fromCoinPoolAmount, toCoinPoolAmount, adjustment);
      expect(output.toPrecision(18)).toEqual(new BigNumber(expected).toPrecision(18));
    },
  );
});
...
describe("reverse swap", () => {
  test.each(reverseSwapFixture.map((x) => [x.S, x.X, x.Y, x.expected]))(
    ...
    (targetAmount, targetCoinPoolAmount, fromCoinPoolAmount, expected) => {
      const x = calculateSwapAmountNeeded(targetAmount, targetCoinPoolAmount, fromCoinPoolAmount);
      expect(x.toPrecision(18)).toEqual(new BigNumber(expected).toPrecision(18));
    },
  );
});
...
describe("provider's fee", () => {
  test.each(providerFeesFixture.map((x) => [x.x, x.X, x.Y, x.expected]))(
    ...
    (fromAmount, fromCoinPoolAmount, toCoinPoolAmount, expected) => {
      const output = calculateLiquidityProviderFee(fromAmount, fromCoinPoolAmount, toCoinPoolAmount);
      expect(output.toPrecision(18)).toEqual(new BigNumber(expected).toPrecision(18));
    },
  );
});
...
describe("price impact", () => {
  test.each(priceImpactFixture.map((x) => [x.x, x.X, x.expected]))(
    ...
    (fromAmount, fromCoinPoolAmount, expected) => {
      ...
      return expect(output.toPrecision(18)).toEqual(new BigNumber(expected).toPrecision(18));
    },
  );
});
